    ...
    Public Sub filtro()
        ...
        Try
            ...
            For x = 1 To 6
                ...
                '  Dim sql As String = "SELECT * FROM Articulos  WHERE Nombre='" & txtbusqueda.Text & "'"
                '  Dim cmd As New SqlCommand(sql, conexion)
                '  cmd.Parameters.AddWithValue("@id", Convert.ToInt32(txt1.Text))

                ' Dim reader As SqlDataReader = cmd.ExecuteReader()

                '  If ' reader.Read() Then
                ' txt1.Text = Convert.ToString(reader("campo1"))
                '  txt1.Text = Convert.ToString(reader("campo2"))
                '  PictureBox1.Image = image.FromFile(Convert.ToString(reader("campoimagen")))

                '  End If



                ' image(x) = New PictureBox
                '  image(x).Image = ds.Tables("Articulos").Rows(a).Item(3)
                ' image(x).Text = "holi"
                ' image(x).Location = New Point(500, 500)
                '  Me.Controls.Add(image(x))
                '  PictureBox1.Image = image(x).Image

                '  panel = New Panel
                '  panel.Size = New Size(150, 100)
                '  panel.Location = New Point(540, 65)
                '   panel.BackColor = System.Drawing.Color.FromArgb(0, 0, 255)
