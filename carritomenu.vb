...
Public Class carritomenu
    ...
    Public verticalfondo As Integer = 5
        ...
        Try
            ...
            For x = 1 To 20
                ...
                If menub.almacena(x) > 0 Then
                    ...
                    descrpcion(x).Text = ds.Tables("Articulos").Rows(a).Item(1)
                    ...
                    botonresta(x).Image = System.Drawing.Image.FromFile("C:\Users\alexa\Desktop\Proyecto comparador de producto\Imagenes\menosx48.png")

                    botonresta(x).BackgroundImageLayout = False
                    botonresta(x).FlatStyle = FlatStyle.Flat
                    botonresta(x).Width = 60
                    botonresta(x).Height = 85
                    botonresta(x).Location = New Point(250, vertical)
                    ...
                    campo(x).Width = 60
                    campo(x).Height = 85
                    campo(x).TextAlign = HorizontalAlignment.Center

                    campo(x).Multiline = True
                    campo(x).Location = New Point(330, vertical)

                    Me.Controls.Add(campo(x))

                    total(x) = New TextBox
                    total(x).Text = campo(x).Text * precio(x).Text
                    total(x).Location = New Point(500, vertical)

                    Me.Controls.Add(total(x))

                    botonsuma(x) = New Button
                    botonsuma(x).Image = System.Drawing.Image.FromFile("C:\Users\alexa\Desktop\Proyecto comparador de producto\Imagenes\masx48.png")

                    botonsuma(x).Width = 60
                    botonsuma(x).Height = 85
                    botonsuma(x).Location = New Point(420, vertical)
    ...
    Private Sub PictureBox1_Click(sender As Object, e As EventArgs) Handles factura.Click
        Facturacion.Show()
    End Sub
End Class